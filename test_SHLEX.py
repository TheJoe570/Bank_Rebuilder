...
def extract_players_and_banks(replay_file_path):
    try:
        command = f"s2repdump -d players -d banks {shlex.quote(replay_file_path)}"
...
def rebuild_bank(replay_file_path):
    try:
        ...
        command = f"s2repdump --bank-rebuild --out {shlex.quote(output_dir)} {shlex.quote(replay_file_path)}"
        logging.info(f"Running command: {command}")
